...
# trust corporate certs on Windows (no-op if unavailable)
try:
...
def main() -> None:
    with httpx.Client(follow_redirects=False, timeout=30.0, trust_env=True) as client:
        ...
        print(f"WWW-Authenticate: {wa!r}")
        sc = r.headers.get("set-cookie", "")
        print(f"Set-Cookie: {'<present>' if sc else '<none>'}")
