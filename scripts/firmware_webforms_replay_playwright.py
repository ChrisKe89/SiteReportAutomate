...
async def select_timezone_on_page(page, desired_value: str, label_hint: str | None = None) -> str:
    """
    ...
    """, desired_value)
    if ok:
        return await page.eval_on_selector('select#MainContent_ddlTimeZone', "el => el.value") or desired_value
...
async def post_search(page, hidden: Dict[str, str], opco: str, product_code: str, serial: str) -> Tuple[int, str]:
    form = {
...
async def dom_submit_schedule(page, opco: str, product_code: str, serial: str,
                              date_iso: str, time_val: str, tz_val: str) -> tuple[int, str]:
    """
    Fill fields in the DOM and trigger a real WebForms postback (robust click; __doPostBack fallback).
    Returns (status_code, parsed_status_text).
    """
    await page.goto(URL, wait_until="domcontentloaded")

    # Fill the inputs/selects; order matters a bit for some validators
    await page.evaluate("""
    ...
    # Give any client-side scripts a tick to update hidden fields
    await page.wait_for_timeout(150)

    # Optional: dump timezone options and selected value
    await debug_dump_timezone(page)

    # Robust submit: try many selectors; else call __doPostBack.
    selectors = [
    ...
    for sel in selectors:
        try:
            ...
            if await locator.first.is_disabled():
                continue
    ...
    if not clicked:
        # Last resort: server-side postback by name
        await page.evaluate("""
            (eventTarget) => {
              if (typeof window.__doPostBack === 'function') {
                window.__doPostBack(eventTarget, '');
              } else {
                const f = document.forms && document.forms[0];
                if (f) f.submit();
              }
            }
        """, SCHEDULE_TRIGGER)

    # Read any status message after UpdatePanel refresh
    async def read_status() -> str:
        for sel in ["#MainContent_MessageLabel", "#MainContent_lblMessage", "#MainContent_lblStatus"]:
...
async def main() -> None:
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=HEADLESS, channel=BROWSER_CHANNEL, args=browser_args)
        ...
        with out_path.open("w", newline="", encoding="utf-8") as fout:
            fieldnames = [
                ...
                "http_status_search","status_text_search",
                "http_status_schedule","status_text_schedule",
                "scheduled_date","scheduled_time","timezone_value",
            ]
            ...
            for item in rows:
                ...
                code_s, html_s = await post_search(page, hidden, item.get("opco") or DEFAULT_OPCO, product, serial)
                ...
                await page.goto(URL, wait_until="domcontentloaded")
                label_hint = "Canberra, Melbourne, Sydney" if desired_tz_val == "+11:00" else None
                actual_tz_val = await select_timezone_on_page(page, desired_tz_val, label_hint)
